...
import React, { useState, useRef, useEffect, useTransition, FormEvent, ChangeEvent, useCallback } from "react";
...
const AVAILABLE_MODES: AiMode[] = [
  { id: "general", name: "General Chat", description: "Engage in a freeform conversation.", icon: Sparkles },
...
export default function ModeChatPage() {
  ...
  const handleNewChat = useCallback((isInitial: boolean = false, modeId: AiModeId = 'general') => {
    ...
    const newSession: ChatSession = {
      id: newSessionId,
      name: isInitial ? "Welcome Chat" : `${mode.name} Chat - ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`,
      messages: [
        {
          id: "ai-init-" + Date.now(),
          sender: "ai",
  ...
  const handleDeleteSession = (sessionId: string) => {
    ...
    if (currentSessionId === sessionId) {
      if (newSessions.length > 0) {
        const sortedRemaining = [...newSessions].sort((a,b) => b.createdAt - a.createdAt);
  ...
  const handleRenameSession = (sessionId: string, newName: string) => {
    setChatSessions(prevSessions =>
      prevSessions.map(s => (s.id === sessionId ? { ...s, name: newName } : s))
    );
  };
        ...
        <div className="flex flex-col h-full bg-background">
          <header className="flex items-center justify-between p-4 border-b sticky top-0 z-10 bg-background">
            <div className="flex items-center gap-2">
              <SidebarTrigger />
              <Image src="/icon.png" alt="Even App Icon" width={32} height={32} priority />
                ...
                {isPending && (
                  <div className="flex justify-start items-start gap-3 py-4">
                    <div className="h-10 w-10 shrink-0 relative">
                       <Image src="/icon.png" alt="AI Icon" layout="fill" objectFit="contain" />
                    </div>
                 ...
                 {!currentSessionId && !isPending && chatSessions.length === 0 && (
          ...
          <footer className="p-4 border-t bg-background sticky bottom-0 z-10">
            ...
            >
              ...
              <Button type="submit" size="icon" disabled={isPending || !inputValue.trim() || !currentSessionId} aria-label="Send message">
